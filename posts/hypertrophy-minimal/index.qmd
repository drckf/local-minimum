...
Fitness-fatigue models are a class of phenomenological model that are widely used to model the response to 
training [@banister1975systems, @busso2003variable, @ceddia2025mathematical, @imbach2022use, @philippe2019modelling]. 
Essentially, the idea is that the response to training reflects a balance 
between two competing processes—a fitness component that improves performance, and a fatigue component that
reduces performance—operating with different intensities and at different timescales. These models describe 
the performance $p(t)$ at time $t$ as 
$$
...
$$
where $\mathrm{TD} = 4\tau_{A1}$ is a time delay.
